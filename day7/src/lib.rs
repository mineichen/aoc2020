...
pub fn read_rules<'a>(parser: &'a mut RuleParser) -> impl Iterator<Item=Rule> + 'a {
    utils::LineReaderIterator::from_file(
        "day7/input.txt", 
        move|i| parser.parse_line(i)).map(Result::unwrap
    )
}
...
impl IdGenerator {
    fn new() -> Self {
        Self { set: std::collections::HashMap::new(), ctr: 1 }
    }
...
impl Rule {
    ...
    pub fn count_with_sub_bags(&self, store: &HashMap::<u64, Rule>) -> u32 {
        1 + self.sub.iter()
            .fold(0, |acc, sub| acc + sub.count * store[&sub.id].count_with_sub_bags(store))
    }
}
...
mod tests {
    ...
    fn contains_recursive_test() {
        ...
        map.insert(1, Rule { 
            id: 1, 
            sub: vec!(
                RuleRequirement { count: 1, id: 2 }
            )
        });
        map.insert(2, Rule { 
            id: 2, 
            sub: vec!(
                RuleRequirement { count: 1, id: 3 }
            )
        });
        map.insert(3, Rule { 
            id: 2, 
