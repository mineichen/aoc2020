...
impl Bag {
    fn new(active: impl Iterator<Item=(i32, i32)>) -> Self {
        ...
        let active_cells = active.map(|a| {
            ...
            Coordinate (a.0, a.1, 0)
        }).collect();
        
        Bag { active_cells, bounds_x, bounds_y, bounds_z}
    }
    pub fn print_slice(&self, z: i32) {
        for y in self.bounds_y.0..=self.bounds_y.1 {
            for x in self.bounds_x.0..=self.bounds_x.1 {
                print!("{}", if self.active_cells.contains(&Coordinate(x, y, z)) { "#" } else { "." });
            }
            print!("\n");
        }
    }
    ...
    fn iter_bounds(&self) -> impl Iterator<Item=Coordinate> + '_ {
        (self.bounds_x.0..=self.bounds_x.1).flat_map(
            move |x| (self.bounds_y.0..=self.bounds_y.1).flat_map(
                move |y| (self.bounds_z.0..=self.bounds_z.1).map(
                    move |z| Coordinate(x, y, z)
                )
            )
        )
    }
...
impl Coordinate {
    fn neighbours(&self) -> impl Iterator<Item=Coordinate> + '_ {
        ((self.2-1)..=(self.2+1)).flat_map(
            move |z| ((self.1-1)..=(self.1+1)).flat_map(
                move |y| ((self.0-1)..=(self.0+1)).filter(move |x| x != &self.0 || y != self.1 || z != self.2).map(
                    move |x| Coordinate(x, y, z)
                )
            )
        )
    }
...
mod tests {
    ...
    #[test]
    fn coordinate_has_26_neighbours() {
        let neighbours = Coordinate(0,0,0).neighbours().collect::<Vec<_>>();
        assert_eq!(26, neighbours.len());

        assert!(neighbours.contains(&Coordinate(-1,-1,-1)));
        assert!(neighbours.contains(&Coordinate(-1, 0,-1)));
        assert!(neighbours.contains(&Coordinate(-1, 1,-1)));
        assert!(neighbours.contains(&Coordinate(0,-1,-1)));
        assert!(neighbours.contains(&Coordinate(0, 0,-1)));
        assert!(neighbours.contains(&Coordinate(0, 1,-1)));
        assert!(neighbours.contains(&Coordinate(1,-1,-1)));
        assert!(neighbours.contains(&Coordinate(1, 0,-1)));
        assert!(neighbours.contains(&Coordinate(1, 1,-1)));
        ...
        assert!(neighbours.contains(&Coordinate(-1,-1, 0)));
        assert!(neighbours.contains(&Coordinate(-1, 0, 0)));
        assert!(neighbours.contains(&Coordinate(-1, 1, 0)));
        assert!(neighbours.contains(&Coordinate(0,-1, 0)));
        // assert!(neighbours.contains(&Coordinate(0, 0, 0)));
        assert!(neighbours.contains(&Coordinate(0, 1, 0)));
        assert!(neighbours.contains(&Coordinate(1,-1, 0)));
        assert!(neighbours.contains(&Coordinate(1, 0, 0)));
        assert!(neighbours.contains(&Coordinate(1, 1, 0)));
    ...
    #[test]
    fn verify_testdata() {
        let mut bag = load("day17/test_input.txt");
        for _ in 0..6 {
            bag.print_slice(0);
            bag.step();
            bag.print_slice(0);
        }
        assert_eq!(112, bag.count_active());
    }
}