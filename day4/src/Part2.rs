...
fn main() {
    println!(
        "Valid passports with rules: {}",
        lib::count_valid_passports(validate_rule)
    );
}

fn validate_rule(flag: lib::Flags, data: &str) -> Result<bool, utils::Error> {
    Ok(match flag {
        Flags::BYR => data.parse::<u32>().map(|n| n >= 1920 && n <= 2002)?,
        Flags::IYR => data.parse::<u32>().map(|n| n >= 2010 && n <= 2020)?,
        Flags::EYR => data.parse::<u32>().map(|n| n >= 2020 && n <= 2030)?,
        Flags::HGT => {
        ...
        Flags::HCL => {
            ...
            match first.chars().next() {
                Some(first_char) => {
                    first_char == '#' && u32::from_str_radix(rest, 16).is_ok() && rest.len() == 6
                }
                None => false,
            }
        }
        Flags::ECL => match data {
